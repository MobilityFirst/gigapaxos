...
const CHECK_ACTIVE_ARs_DURATION = 30 * 1000;
...
function removeARAndChooseNextBestReplica(serviceName) {
  ...
  return latenciesMap.get(serviceName).get('latencies').size > 0
    ? getARWithLeastLatency(latenciesMap.get(serviceName).get('latencies'))
    : null;
}
...
export default async function sendRequest(
  ...
  if (!isInitialized.has(serviceName)) {
    const service = {
      ...
      isLocal: rcUrls && rcUrls.length > 0 ? (rcUrls[0].includes('127.0.0.1') ? true : false) : true,
    };
  ...
  return response == null ? NO_ACTIVE_REPLICAS : response;
}
...
async function initService(service) {
  ...
  latenciesMap.get(serviceName).set(
    'reconfigurators',
    rcUrls.map((x) => new InetSocketAddress(x.split(':')[0], x.split(':')[1])),
  );
  ...
  if (!latenciesMap.get(serviceName).has('intervalId')) {
    ...
    latenciesMap.get(serviceName).set(
      ...
      setInterval(function () {
        periodicallyCheckActiveARs(serviceName);
      }, CHECK_ACTIVE_ARs_DURATION),
    );
  }
